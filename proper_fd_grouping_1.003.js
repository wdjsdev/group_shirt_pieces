...
function container()
{
	...
	function fixCompoundPaths()
	{
		...
		var leftToProcess = cPaths.length;
		for(var a=0;a<cPaths.length;a++)
		{
			var thisCPath = cPaths[a];
			if(thisCPath.layer.name == "Guides" || thisCPath.layer.name == "BKGRD, do not unlock" || thisCPath.layer.name == "USA Collars")
			{
				continue;
			}
			...
			leftToProcess--;
			$.writeln(leftToProcess + " cPaths left to process.");
		}
	}
