...
Progress:
	...
	Version 1.005
		05 January, 2017
		Building batch function and commenting duration calculations

	Version 1.006
		10 January, 2017
		Added support for live text frames

	Version 1.007
		03 February, 2017
		Added pocket welt colors to list of art colors.
...
function container()
{
	...
	function fixCompoundPaths()
	{
		...
		//uncomment to calculate duration
	...
	function checkPaths(thisGroup)
	{
		...
		for(var c = thisGroup.pageItems.length-1;c >-1; c--)
		{
			...
			//TextFrame
			//TextFrame
			else if(thisItem.typename == "TextFrame")
			{
				var dest = getDest(thisItem);
				thisItem.moveToBeginning(dest);
			}
	...
	function getDest(path)
	{
		...
		//get fill and stroke color typenames
		if(path.typename == "PathItem")
		{
			var fillType = path.fillColor.typename;
			var strokeType = path.strokeColor.typename;
		}
		else if(path.typename == "TextFrame")
		{
			var fillType = path.textRange.characterAttributes.fillColor.typename;
			var strokeType = path.textRange.characterAttributes.strokeColor.typename;
			isText = true;
		}
		...
		switch(fillType)
		{
			case "SpotColor" :
				...
				{
					var fillName = path.textRange.characterAttributes.fillColor.spot.name.toLowerCase();
				}
		...
		switch(strokeType)
		{
			case "SpotColor" :
				...
				{
					var strokeName = path.textRange.characterAttributes.strokeColor.spot.name.toLowerCase();
				}
		...
		function getColor(thisItem)
		{
			...
			switch(thisItem.typename)
			{
				...
				case "CompoundPathItem" :
					...
					break;
				case "TextFrame" :
					if(thisItem.textRange.characterAttributes.fillColor != "NoColor")
					{
						thisColor = thisItem.textRange.characterAttributes.fillColor.spot.name;
					}
					else if(thisItem.textRange.characterAttributes.strokeColor != "NoColor")
					{
						thisColor = thisItem.textRange.characterAttributes.strokeColor.spot.name;
					}
	...
	var lib = 
	{
		...
		"artColors" : "collar b, collar 2 b, collar info b, boombah logo b, boombah logo 2 b, pocket facing, pocket welt 1, pocket welt 2",
